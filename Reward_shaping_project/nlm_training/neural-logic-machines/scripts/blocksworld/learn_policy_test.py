...
K_PLAYER = {i:0 for i in range(args.test_number_begin, args.test_number_end+args.test_number_step, args.test_number_step)}
class MyTrainer(MiningTrainerBase):
  ...
  def _get_player(self, number, mode):
    ...
    player.restart()
    test_players[number] += [copy.deepcopy(player)]
    
    '''
    player = test_players[number][K_PLAYER[number]]
    #player.restart()
    #print((number,K_PLAYER[number]),player.unwrapped.start_state)
    K_PLAYER[number] += 1 #% len(test_players[number])
    '''
  ...
  def _get_result_given_player(self, index, meters, number, player, mode):
    ...
    #print('argmax:',params['use_argmax'])
...
if __name__ == '__main__':
  ...
  '''
  test_players = {}
  with open('test_results/bw_test_instances.pickle', 'rb') as handle:
    test_players = pickle.load(handle)
  '''
  test_players = {i:[] for i in range(args.test_number_begin, args.test_number_end+args.test_number_step, args.test_number_step)}
  
  EPOCHS = [9]
  for j in EPOCHS:
      args.dump_dir = 'test_results/'
      args.load_checkpoint = 'training_cpbp_1087/'+'saves/'+'checkpoint_'+str(j)+'0.pth'
      for i in range(args.runs):
        graduated, test_meters = main(i)
        logger.info('run {}'.format(i + 1))
        
        with open('test_results/bw_test_instances.pickle', 'wb') as handle:
            pickle.dump(test_players, handle, protocol=pickle.HIGHEST_PROTOCOL)
