...
def get_coordinates(world, absolute=False, free_space=None):
  ...
  #fs_indicateor = np.ones(((coordinates).shape[0],1))*free_space
  '''
  if(free_space == None or free_space==0):
      fs_indicator = np.zeros(((coordinates).shape[0],1))
  else:
      fs_indicator = np.ones(((coordinates).shape[0],1))
  
  
  k=2
  for i in range(min(2,free_space)):
    fs_indicator = np.ones(((coordinates).shape[0],1))
    coordinates = np.append(coordinates, fs_indicator, axis=1)
    k-=1
  for i in range(k):
    fs_indicator = np.zeros(((coordinates).shape[0],1))
    coordinates = np.append(coordinates, fs_indicator, axis=1)
