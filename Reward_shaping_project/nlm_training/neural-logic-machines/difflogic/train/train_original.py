...
class TrainerBase(TrainerEnv):
  ...
  def _get_best_thresholds(self):
    min_vars = None
    min_threshold=None
    for i_threshold in range(50,100+1):
        vars,_,_,_, _ = get_variance(self.grad_data, flat_mode = False, mode='cheapest', threshold=i_threshold/100 )
        if((min_vars == None) or (vars<min_vars)):
            min_threshold = i_threshold
            min_vars      = vars
        #print(i_threshold, ': ',vars)
  ...
  def _train_epoch(self, epoch_size):
    ...
    succ = self.get_succ()
    print('succ:', self.get_succ())
    if(succ>0):
        ...
        #self.min_threshold = self._get_best_thresholds()  
    else:
        self.min_threshold = 100
    print('threshold:',self.min_threshold,'%')
    return meters
...
class MiningTrainerBase(CurriculumTrainerBase):
  ...
  def _get_result(self, index, meters, mode):
    ...
    player.unwrapped.set_threshold(self.min_threshold)
...
def get_variance(grad_data, threshold=1.0, flat_mode = False, mode='cheapest'):
    ...
    vars = np.mean(np.var(grads_np,axis=0))*1
