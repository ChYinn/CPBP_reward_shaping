...
class TrainerBase(TrainerEnv):
  ...
  def __init__(self, model, optimizer, epochs, epoch_size, test_epoch_size,
    ...
    self.last_mining_epoch = None
    self.lr = None
    self.entropy_beta = None
    self.threshold = 100
    self.baseline_lengths = {l:{} for l in range(test_number_end+1)}
    self.trainer_baseline = None
    ...
    self.optim = None
    self.grad_data = []
    self.grad_tune_len = 40
    self.grad_tune_data = {i:deque(maxlen=self.grad_tune_len) for i in range(1,12+1)}
    self.grad_tune_period = 20
    self.Es = {i:deque(maxlen=i*(4*i*100)) for i in range(1,12+1)}
    self.min_threshold = {i:1.0 for i in range(1,12+1)}
  ...
  def _get_result(self, index, meters, mode):
    ...
    """
    raise NotImplementedError()
  def _get_best_thresholds(self, number):
    if(len(self.grad_tune_data[number])==0):
        return 1.0
    #return 100
    min_vars = None
    min_threshold=None

    thresh_grid =  [i/100 for i in range(1,101)]
    thresh_grid += [i/10000 for i in range(1,100)]
    #print(thresh_grid)
      
    for i_threshold in thresh_grid:
        vars,_,_,_, _ = get_variance(list(self.grad_tune_data[number]), flat_mode = False, mode='cheapest', threshold=i_threshold )
        if((min_vars == None) or (vars<min_vars)):
            min_threshold = i_threshold
            min_vars      = vars
    return min_threshold
      
  def _train_step(self, feed_dict, meters):    
    #super().save_checkpoint(os.path.join(self.checkpoints_dir,'trainer_params.pth'))
    #self.trainer_baseline.load_checkpoint(os.path.join(self.checkpoints_dir,'trainer_params.pth'))
    #ret             = self.step(feed_dict,  grad_clip=0.0001)
    ret             = self.step(feed_dict)
    #ret             = self.step(feed_dict)
    #grad_info   = self.trainer_baseline.step(feed_dict, redundancy=True)
    grad_info   = None
    loss, monitors, output_dict, extras = ret
    meters.update(monitor_gradrms(self.model))
    meters.update(monitors)
    meters.update(loss=loss)
    if(not grad_info == None):
        self.grad_data += [(grad_info,feed_dict['succ'],feed_dict['number'], len(feed_dict['rewards']))]
        if(feed_dict['succ'] or True):
            self.grad_tune_data[feed_dict['number']].append(self.grad_data[-1])

    
    return 'Train: loss={loss:.4f}'.format(loss=loss), ret
  '''
  def get_succ(self):
      grad_data = self.grad_data
      n_succs = 0
      for i in range(len(grad_data)):
        grad_info   = grad_data[i]
        succ        = (grad_info[1])
        if(succ):
            n_succs+=1
      print('len:',len(grad_data))
      return n_succs/len(grad_data)
  '''
  def update_threshold(self):
      best_threshes = ''
      for num in self.sample_array:
          self.min_threshold[num] = self._get_best_thresholds(num)*0.2 + self.min_threshold[num]*0.8
          best_threshes += str(num)+':'+str(self.min_threshold[num]*100)+'%, '
      print(best_threshes)
  def _train_epoch(self, epoch_size):
    ...
    def train_func(index):
      ...
      if(index%self.grad_tune_period==0):
          ...
    ...
    self.threshold = 100-int((meters.avg['succ']*100)*0.5)
    #assert(meters.avg['succ']>0)
    self.baseline_length = meters.avg['length']
    io.mkdir(self.checkpoints_dir+'/grad_data')
    checkpoint_file = self.checkpoints_dir+'/grad_data/grad_data_' +str(self.current_epoch)+'.pickle'
    
    with open(checkpoint_file, 'wb') as handle:
        pickle.dump(self.grad_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
    #self.update_threshold()
...
class CurriculumTrainerBase(TrainerBase):
  ...
  def _get_threshold(self):
    #MODIF
    #return 1.0
    if(self.curriculum_graduate==self.current_number):
        return 1.0
    return self.curriculum_thresh - self.curriculum_thresh_relax * \
        (max(0,(self.curriculum_graduate)- self.current_number))
  ...
  def _upgrade_lesson(self, update_number=True):
    """Upgrade to next lesson."""
    if(update_number):
        self.nr_upgrades += 1
    self.last_upgrade_epoch = self.current_epoch
    if self.enable_candidate:
      print('ready_to_graduate:',self.ready_to_graduate,'current_number:',self.current_number,'is_candidate:',self.is_candidate)
      ...
      elif (self.is_candidate and self.ready_to_graduate):
  ...
  def _train_epoch(self, epoch_size):
    ...
    if(self.current_number == self.curriculum_graduate):
        self.ready_to_graduate = True
    else:
        self.ready_to_graduate = True
    print('<!> ready to graduate?',self.ready_to_graduate, ', current number:',self.current_number)
    self.n_update_consecutive=0
...
class MiningTrainerBase(CurriculumTrainerBase):
  ...
  def _get_result(self, index, meters, mode):
    ...
    player.unwrapped.set_threshold(self.min_threshold[number])
    player.unwrapped.set_baseline_lengths(self.baseline_lengths)
    player.unwrapped.es_history = self.Es[number]
    if(not mode=='train'):
        return self._get_result_given_player(index, meters, number, player, mode)
    else:
        a,b = self._get_result_given_player(index, meters, number, player, mode)
        for i in range(len(b)):
            self.Es[number].append(b[i])
        
        if(not b==None):
            self.baseline_lengths=b
        #print(self.baseline_lengths[player.unwrapped.nr_blocks])
        return a
  ...
  def _take_exam(self, train_meters=None):
    if(self.n_update_consecutive>=2 and False):
        self.need_mining=False
        return
    else:
        self.n_update_consecutive += 1
    ...
    print("mining_interval:"    ,self.mining_interval,
          ",current_epoch:"     ,self.current_epoch,
          ",last_mining_epoch:" ,self.last_mining_epoch,
          ",need_mining:"        ,self.need_mining,
          ",warm_start:"         ,self.warm_start)
    
    if self.need_mining or (self.mining_interval <=
                            self.current_epoch - self.last_mining_epoch) or (self.warm_start):
      ...
      if(self.n_update_consecutive==1):
          print('succ (curr number):',meters.avg['succ'])
          if(meters.avg['succ']==0):
              self.strike += 1
          else:
              ...
              #self.strike  = 0
          print('n. strikes:',self.strike)
          assert(self.strike<1)
      # Use the performance during mining as the outcome for the exam.
      if self._pass_lesson(meters):
        self.strike  = 0
        super().save_checkpoint(os.path.join(self.checkpoints_dir,'best_trainer.pth'))
        self._upgrade_lesson()
        #self.min_threshold = 1.0

        '''
        for num in self.sample_array:
            new_grad_tune_data = deque(maxlen=self.grad_tune_len)
            #print('before:',len(self.grad_tune_data))
            
            for k in range(len(self.grad_tune_data[num])):
                grad_info = self.grad_tune_data[num].pop()
                nBlocks      = (grad_info[2])
                #if((nBlocks-1) in self.sample_array):
                    new_grad_tune_data.append(grad_info)
            self.grad_tune_data=new_grad_tune_data
            print('after:',len(self.grad_tune_data))
            
        '''
        #self.grad_tune_data.clear()
        ...
        #self.strike += 1
        self.need_mining = False
        #assert(self.strike<10)

  def train(self):
    self.need_mining = False
    if(self.last_mining_epoch is None):
        self.last_mining_epoch = 0    
    return super().train()


def get_variance(grad_data, threshold=1.0, flat_mode = False, mode='cheapest'):
    i_mode=None
    if(mode=='expensive'):
        i_mode=0
    elif(mode=='mid'):
    ...
    for i in range(len(grad_data)):
        ...
        distrib_final = grad_info[0][-1][1]
        final_E = get_expected_value(distrib_final[i_mode], threshold=threshold, flat_mode=flat_mode, n_actions_taken = (grad_info[3]))
        
        succ         = (grad_info[1])
        nBlocks      = (grad_info[2])
        nActions     = (grad_info[3])
        final_E      = 2*nBlocks
        ...
        if(succ):
            ...
            ...
            #print(succ,final_E,'vs.',2*nBlocks)
        
        for j_time in range(len(grad_info[0])-1):
            gradient     = (grad_info[0][j_time][0])
            distribution = (grad_info[0][j_time][1])
            skip_grad = False
            es += [get_expected_value(distribution[i_mode], threshold=threshold, flat_mode=flat_mode, n_actions_taken = j_time)]#-j_time]
            
            if(not succ):
                rets += [nActions-j_time+final_E]
            else:
                rets += [nActions-j_time]
            
            #if(succ or True)::
            grads_with_baseline      += [np.array(gradient)*(rets[-1]-es[-1])/(nBlocks)]#(np.abs(es[-1]))]
            
            #grads_with_baseline      += [np.array(gradient)*(final_E-es[-1])/(np.abs(es[-1]))]
            #grads_no_baseline   += [np.array(gradient)*(rets[-1])]
            #if(i==11 and succ):
            #    print('R'+str(j_time)+':',rets[-1],'E['+'R'+str(j_time)+']:',es[-1],' (%):',threshold)
            #if(succ):
            #    grads_with_baseline      += [np.array(gradient)*(rets[-1]-es[-1])/(nBlocks)]
            #else:
            #    grads_with_baseline_fail += [np.array(gradient)*(rets[-1]-es[-1])/(nBlocks)]
    #if(not succ):
    #    print((rets[-1]),'vs.',(es[-1]),'len:',len(grad_info[0])-1,'nActions:',nActions)
            
    grads_np = (np.array(grads_with_baseline))
    varis = np.mean(np.var(grads_np,axis=0))*1
    
    return (varis),0, np.mean(succs), 0, 0


'''
def get_variance(grad_data, flat_mode = False, mode='cheapest', threshold=1.0):
    i_mode=None
    if(mode=='expensive'):
        i_mode=0
    elif(mode=='mid'):
    ...
    for i in range(len(grad_data)):
        ...
        for j_time in range(len(grad_info[0])):
            gradient     = (grad_info[0][j_time][0])
            distribution = (grad_info[0][j_time][1])
            #succ         = (grad_info[1])
            nBlocks      = (grad_info[2])
            nActions     = (grad_info[3])
            skip_grad = False
            es += [get_expected_value(distribution[i_mode], threshold=threshold, flat_mode=flat_mode)-j_time]
            #print(a3)
            if(not succ):
                rets += [nActions-j_time+nBlocks*2]
            ...
            if(succ):
                grads_with_baseline += [np.array(gradient)*(rets[-1]-es[-1])/nBlocks]
            #else:
            #    grads_with_baseline += [np.array(gradient)*(rets[-1]-es[-1])/(nBlocks*20)]
            X += [np.array(gradient)*(rets[-1])/nBlocks]
            #Y += [np.array(gradient)*(es[-1])/nBlocks]
            Y += [np.array(gradient)]
        if(nBlocks==4 and (threshold==0.01) and False):
            print(es)
            print(rets)
            print(nBlocks)
            print('----')
        ...
        #print('--')
        #vars_no_baseline += [(np.var(grads_no_baseline))]
        #vars_with_baseline    += [(np.var(grads_with_baseline))]
    #print(succs)
    #print(np.mean(succs))
    grads_np = (np.array(grads_with_baseline))
    vars = np.mean(np.var(grads_np,axis=0))
    #vars_err = np.std(np.var(grads_np,axis=0)) = 0  
    #Y = np.array(Y)
    #print(Y.shape)
    #Y = np.mean(Y,axis=1)
    return vars,0, np.mean(succs), np.mean(Y), np.mean(X)
'''
'''
def get_expected_value(distribution, threshold=1.0, flat_mode=False):
