...
class BlocksWorld(AI_Plan):
            ...
            for j in range(self.nb_blocks+1):
                if(i==j or i==self.nb_blocks):
                    ...
                    if(j==self.nb_blocks):
                        j_='ground'
                    if(j_=='ground'):
                        #keys[(i,j_,'added')] = len(actions)
                        #actions += [(i,j_,'added')]
                        keys[(i,j_,'dummy')] = len(actions)
                        actions += [(i,j_,'dummy')]
                    else:
                        #keys[(i,j_,'lifted')] = len(actions)
                        #actions += [(i,j_,'lifted')]
                        keys[(i,j_,'dummy')] = len(actions)
                        actions += [(i,j_,'dummy')]
        ...
        for i in range(len(self.global_actions)):
            ...
            if(a_i_global=='stay' or a_i_global=='finish'):
                ...
                if(not(a_i_global[0]==block_i or a_i_global[1]==block_i)):
                    if(not (a_i_global[0],'dummy','dummy') in actions_local):
                        map_global_to_local[i] = len(actions_local)
                        actions_keys_local[(a_i_global[0],'dummy','dummy')] = len(actions_local)
                        actions_local += [(a_i_global[0],'dummy','dummy')]
                    else:
                        map_global_to_local[i] = actions_keys_local[(a_i_global[0],'dummy','dummy')]
                else:
                    map_global_to_local[i] = len(actions_local)
                    actions_keys_local[a_i_global] = len(actions_local)
                    actions_local += [a_i_global]
    ...
    def get_local_states(self, block):
        ...
        for i in range(self.nb_blocks+1):
            ...
            #states_local                    += [(i_,'air')]
            states_keys_local[(i_,'dummy','dummy')]     = len(states_local)
            states_local  += [(i_,'dummy','dummy')]
            '''
    ...
    def create_automatons(self):
        ...
        for i in range(0, self.nb_blocks):
                    ...
                    if(state_local=='accept_finished'):
                        ...
                        if(action_local=='finish'):
                            ...
                            if  (state_local[0]== 'air'):
                                if(action_local[0] == i):
                                    '''
                                    if(state_local[2]=='is_ground' and (not action_local[1]=='ground') and (action_local[2]=='lifted')):#lifting
                                    ...
                                    next_state = (action_local[0], 'dummy', 'dummy')
                                    next_cost  = 1
                                else:
                                    next_state = state_local
                                    next_cost = 1
                                ...
                                if(action_local[1]==i):
                                    ...
                                    next_state = 'reject'
                                    next_cost = 0
                                elif(action_local[0]==state_local[0]):
                                    next_state = ('air','dummy', 'dummy')
                                    next_cost = 1
                                else:
                                    next_state = state_local
                                    next_cost = 1
            ...
            if(init_config_tuple[i][1]==goal_config_tuple[i][1]):
                init_i = (init_config_tuple[i][0], 'dummy', 'dummy')
            else:
                init_i = (init_config_tuple[i][0], 'dummy', 'dummy')
