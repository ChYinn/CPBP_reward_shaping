...
public class ClassicalAIplanning {
    ...
    public static Automaton_2D_cost[] automaton;
    public static int maxActionCost;
    public static int nbOptimizationConstraints;
    public static int currentBestPlanCost;
    public static int timeout = 100000; // to look for a plan of given length, in milliseconds
    public static int failout = 100000; // to look for a plan of given length
	
    public void solve() {

	//###########################################
	// read the instance, whose name is the first argument
	InputReader reader = new InputReader("./src/main/java/minicpbp/examples/data/ClassicalAIplanning/problem.txt");
	...
	for(int i=0; i<nbAutomata; i++) {
		...
		for (int j = 0; j < localActionCost.length; j++) {
			for (int k = 0; k < localActionCost[j].length; k++) {
	...
	// try plans of increasing length
 	for (int length = minPlanLength; length <= maxPlanLength; length++) {
		...
		*/
		//if(currentBestPlanCost<maxPlanLength * maxActionCost + 1)
		//break;
		...
		IntVar planCost = makeIntVar(cp, -nbAutomata, maxPlanLength);
		System.out.println(nbAutomata);
		System.out.println(maxPlanLength);
		IntVar[] automataCosts = new IntVar[nbOptimizationConstraints];
		int k = 0;
		// for each component of factored transition system...
		System.out.println("Trying length "+Integer.toString(length));
		for(int i=0; i<nbAutomata; i++) {
			//System.out.println("Best plan: "+Integer.toString(currentBestPlanCost));
			//System.out.println("Accept states: "+automaton[i].goalStates().toString());
			...
			if (objectiveCombinator >= 2) {
			    IntVar automatonCost = makeIntVar(cp, -nbAutomata, maxPlanLength);
		...
		search.onSolution(() -> {
			try {
				File file = new File("./src/main/java/minicpbp/examples/data/ClassicalAIplanning/solution.txt");
				if (!file.exists()) {
					file.createNewFile();
				}
				...
				for (int i = 0; i < action.length; i++) {
					if(i<action.length-1)
						bw.write(Integer.toString(action[i].min())+", ");
					else
						bw.write(Integer.toString(action[i].min()));
				}
				bw.flush();
			...
			currentBestPlanCost = planCost.min();
 			...
 			System.out.println("cost: "+Integer.toString(planCost.min()));
			
			for (int i = 0; i < action.length; i++) {
			    System.out.print(action[i].min()+" ");
			}
			currentBestPlanCost = planCost.min();
