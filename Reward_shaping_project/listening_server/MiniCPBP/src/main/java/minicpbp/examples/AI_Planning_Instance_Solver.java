 ...
 */
public class AI_Planning_Instance_Solver {
    ...
    public int timeout = 10000; // to look for a plan of given length, in milliseconds
    public int failout = 10; // to look for a plan of given length
	...
	public int plan_length_found;
	public AI_Planning_Instance_Solver(String directory_, int planSize_, int nBlocks_, int[] configuration_init_, int[] configuration_target_){
	...
	public void setActions(int[] action){
		...
		for (int i = this.currentIndex+nStays; i < action.length; i++) {
	...
	//###########################################
	currentBestPlanCost = 100000;//maxPlanLength * maxActionCost + 1; // trivial strict upper bound
	//###########################################
	// try plans of increasing length
	//int length = planSize+1;
	for (int length = 36; length <= 100000; length++) {
		if(currentBestPlanCost<100000){
			this.plan_length_found = currentBestPlanCost;
			break;
		}
        //int length = planSize;
		// define the CP model
		...
		IntVar planCost = makeIntVar(cp, 0, currentBestPlanCost - 1);
		this.cost =  planCost;
